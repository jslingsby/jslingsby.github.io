...
monthvals <- rbind(mmeans, m2015, m2016, m2017, m2018) 
monthvals$Time <- c(rep("Mean (1961 to current)", 12), rep("2015", 12), rep("2016", nrow(m2016)), rep("2017", nrow(m2017)), rep("2018", nrow(m2018)))
...
monthvalsR <- rbind(sumsR, m2015, m2016, m2017, m2018) 
monthvalsR$Time <- c(rep("Mean (1945 to current)", 12), rep("2015", 12), rep("2016", nrow(m2016)), rep("2017", nrow(m2017)), rep("2018", nrow(m2018)))
monthvalsR$Missing <- ""
...
g <- g + geom_bar(stat="identity", position="dodge", na.rm=TRUE) + scale_fill_manual(values = c("#4393C3","#D6604D","forestgreen","darkblue","darkgrey")) + ggtitle("Monthly Streamflow") + ylab(bquote("Streamflow (ML)")) 
...
ggplot(monthvalsNA, aes(x = Month, y = Cumulative_Streamflow, colour=Time)) + geom_point() + scale_colour_manual(values = c("#4393C3","#D6604D","forestgreen","darkblue","darkgrey")) + ylab(bquote("Cumulative Streamflow (ML)"))
```
...
g <- g + geom_bar(stat="identity", position="dodge") + scale_fill_manual(values = c("#4393C3","#D6604D","forestgreen","darkblue","darkgrey")) + ggtitle("Monthly Rainfall") + ylab("Rainfall (mm)") + geom_text(data=monthvalsR, aes(label=Missing), vjust=0, hjust=1)
...
```{r, echo=FALSE, fig.align="center"}
ggplot(monthvalsR, aes(x = Month, y = Cumulative_Rainfall, colour=Time)) + geom_point() + scale_colour_manual(values = c("#4393C3","#D6604D","forestgreen","darkblue","darkgrey")) + ylab(bquote("Cumulative Rainfall (mm)"))
```
